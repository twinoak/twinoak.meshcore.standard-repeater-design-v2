...
* **2026-08-14 preflight-2 fixes:** conn-A VBAT_RAW clearance at the TPS63001 rerouted (was 0.111 mm — under JLC's 0.127 mm minimum — now ≥ 0.2 mm, re-verified). Walter U3 ideal diode LCSC swapped **C2869734 → C2832141** (LM66100DCKT, same part/footprint — DCKR fell to 9 pcs JLC stock). Rotation-offset fields unified everywhere to the FT-recognized name `FT Rotation Offset`, schematic and PCB in agreement: RAK U3 = **+90°**, RAK U5 = **180°**, conn-B U1 = **−90°** (all verified against the JLC pink-dot preview; conn-B U1's field added to the schematic so a future field re-sync can't drop the PCB value). Walter adapter: **"USB-C THIS END" silk hint** added at the socket's pin-1/VIN end (USB-C end of the module — the 2×14 socket is unkeyed). Fabrication Toolkit `EXCLUDE DNP` now ticked on all four assembled boards. Stock watch at order time: RAK4630 47 pcs, LPS4018 (conn-A L2) 12 pcs, SI4840DY 152 pcs, CAX toggle via Global Sourcing. USB-C receptacle vs 1.6 mm board: **decided 2026-08-14 — keeping the TYPEC-303-ACP16 (C720628) on the standard 1.6 mm board as-is** (shell tabs solder from the top into the slots; maintenance-port duty). Same-day layout polish: conn-B spine widened to 40 mil (2 A path) / 30 mil (panel chain); conn-A TPS63001 input caps moved tight to VIN; RAK I2C pull-ups moved onto the live bus (U2 pins 7/8, vacated D-row pins NC'd); the GND via near the RAK USB-C shell slot relocated (1.49 mm hole-edge gap now); **D-sub SH shield pads tied to GND on both connector boards** (SH pin added to the DE9 symbol, pads picked up by the GND pours). All five boards re-verified: zero opens, fills fresh, no different-net spacing under 0.2 mm outside the accepted RAK USB pair region.
